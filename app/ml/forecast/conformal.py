...
class MondianCQRCalibrator:
    ...
    Stratifies calibration residuals on (station_id, hour_bin) where
    hour_bin = local_hour // 6. Per-stratum q_hat gives tighter intervals
    in cool regimes and wider intervals in hot/afternoon regimes, improving
    the danger/normal PI width ratio while keeping per-stratum coverage.
    """

    def __init__(self) -> None:
        self.alpha: float = 0.10
        self._stratum_q: dict[tuple, float] = {}   # (station_id, hour_bin) -> q_hat
    ...
    def fit(
        ...
        # Per-stratum
        unique_strata = set(zip(station_ids, hour_bins))
        for sid, hbin in unique_strata:
            mask = (np.asarray(station_ids) == sid) & (hour_bins == hbin)
            s = scores[mask]
            n_s = len(s)
            if n_s < 10:
                continue  # too few samples — rely on global fallback
            lv = min(1.0, np.ceil((n_s + 1) * (1 - alpha)) / n_s)
            self._stratum_q[(str(sid), int(hbin))] = float(np.quantile(s, lv))
    ...
    def adjust(
        ...
        for i, (sid, hbin) in enumerate(zip(station_ids, hour_bins)):
            key = (str(sid), int(hbin))
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "stratum_q": {f"{k[0]}|{k[1]}": v for k, v in self._stratum_q.items()},
        }
    ...
    def from_dict(cls, d: dict) -> "MondianCQRCalibrator":
        ...
        for k, v in d.get("stratum_q", {}).items():
            parts = k.split("|", 1)
            obj._stratum_q[(parts[0], int(parts[1]))] = float(v)
        return obj
