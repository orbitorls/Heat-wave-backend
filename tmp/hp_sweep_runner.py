"""Quick HP sweep runner for fold0.
Writes per-trial JSON and logs to tmp/cv_results/hp_sweep.
"""
...
def run_trial(trial_id: int, lr: float, weight_decay: float, batch_size: int,
              epochs: int = 6, patience: int = 2, seq_len: int = 7, future_seq: int = 2,
              num_workers: int = 2, cv_dir: str = 'tmp/cv_splits', results_dir: str = 'tmp/cv_results/hp_sweep'):

    results_dir = Path(results_dir)
    results_dir.mkdir(parents=True, exist_ok=True)
    trial_json = results_dir / f'trial_{trial_id}.json'
    trial_log = results_dir / f'trial_{trial_id}_train.log'
    trial_model = results_dir / f'trial_{trial_id}_model.pt'

    with open(trial_log, 'w', encoding='utf-8') as lf, redirect_stdout(lf), redirect_stderr(lf):
        print(f"=== Trial {trial_id} | lr={lr:.2e} wd={weight_decay:.1e} bs={batch_size} ===")
        start = time.time()

        # Load fold metadata (we'll target fold 0)
        cv_dir = Path(cv_dir)
        ...
        print(f'Using train indices: {train_idx_file} and val indices: {val_idx_file}')

        train_time_indices = load_indices(train_idx_file)
        val_time_indices = load_indices(val_idx_file)

        # Load raw dataset
        print('Loading ERA5 dataset (this may take a while) ...')
        loader = ERA5Loader()
        ds = loader.load_era5()
        ...
        print(f'Raw train samples: {len(train_time_indices)}, mapped train samples: {len(train_samples)}')
        print(f'Raw val samples: {len(val_time_indices)}, mapped val samples: {len(val_samples)}')
        ...
        # Create sequences and select samples
        ...
        # Accumulators for final metrics (will be set each epoch)
        ...
        print(f'Trial {trial_id} finished in {elapsed:.1f}s, epochs_trained={epochs_trained}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results-dir', type=str, default='tmp/cv_results/hp_sweep')
    ...
    for i in range(args.trials):
        ...
        try:
            res, logpath = run_trial(i, lr, weight_decay, batch_size, epochs=6, patience=2, seq_len=7, future_seq=2, num_workers=2, cv_dir=args.cv_dir, results_dir=str(results_dir))
            res['trial'] = i
            results.append(res)
        except Exception as e:
            # Write minimal failure JSON
            err = {'trial': i, 'error': str(e), 'lr': lr, 'weight_decay': weight_decay, 'batch_size': batch_size, 'timestamp': datetime.datetime.utcnow().isoformat() + 'Z'}
            (results_dir / f'trial_{i}.json').write_text(json.dumps(err, indent=2), encoding='utf-8')
            with open(results_dir / f'trial_{i}_train.log', 'a', encoding='utf-8') as lf:
                lf.write('\nERROR:\n')
                lf.write(str(e))
            results.append(err)

    # pick best by lowest val_RMSE (ignore nan and errors)
    best = None
    for r in results:
